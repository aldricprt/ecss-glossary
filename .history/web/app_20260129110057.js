...
function getAllUniqueTags(){
  ...
  for(const ref of referencesData){
    if(ref.tags && Array.isArray(ref.tags)) for(const t of ref.tags) tags.add(t);
  }
...
function renderTagFiltersInto(containerId, clearBtnId){
  ...
  for(const tag of tags){
    ...
    btn.addEventListener('click', ()=>{
      ...
      }else if(currentView === 'references'){
...
document.addEventListener('DOMContentLoaded', async ()=>{
  ...
  const globalResultsContainer = document.getElementById('globalResults');
  ...
  function buildGlobalResults(query){
    ...
    for(const it of glossary){
      if(matchesQuery(it.term, qn) || matchesQuery(it.definition, qn) || matchesQuery(it.abbreviation, qn)){
        results.push({
          type: 'Glossary',
          title: it.term || '',
          subtitle: it.abbreviation ? it.abbreviation : '',
          snippet: it.definition || '',
          action: ()=>{
            const menu = document.getElementById('menuGlossary');
            menu && menu.click();
            q.value = it.term || '';
            doSearch && doSearch();
          }
    ...
    for(const img of imagesData){
      if(matchesQuery(img.title, qn) || matchesQuery(img.original, qn) || matchesQuery(img.filename, qn)){
        results.push({
          type: 'Diagrams',
          title: img.title || img.original || img.filename || '',
          subtitle: img.original || '',
          snippet: img.filename || '',
          action: ()=>{
            const menu = document.getElementById('menuDiagrams');
            menu && menu.click();
          }
        });
      }
    ...
    for(const eq of equationsData){
      if(matchesQuery(eq.name, qn) || matchesQuery(eq.content, qn) || matchesQuery(eq.description, qn)){
        results.push({
          type: 'Equations',
          title: eq.name || '',
          subtitle: '',
          snippet: eq.description || eq.content || '',
          action: ()=>{
            const menu = document.getElementById('menuEquations');
            menu && menu.click();
          }
        });
      }
    ...
    for(const ref of referencesData){
      if(matchesQuery(ref.title, qn) || matchesQuery(ref.author, qn) || matchesQuery(ref.description, qn) || matchesQuery(ref.url, qn)){
        results.push({
          type: 'References',
          title: ref.title || '',
          subtitle: ref.author ? `by ${ref.author}` : '',
          snippet: ref.description || ref.url || '',
          action: ()=>{
            const menu = document.getElementById('menuReferences');
            menu && menu.click();
          }
        });
      }
    ...
    for(const m of methodsData){
      if(matchesQuery(m.title, qn) || matchesQuery(m.definition, qn)){
        results.push({
          type: 'Methods',
          title: m.title || '',
          subtitle: '',
          ...
          action: ()=>{
            const menu = document.getElementById('menuMethods');
            menu && menu.click();
          }
        });
      }
  ...
  function renderGlobalResults(items){
    ...
    for(const item of items.slice(0, 50)){
      ...
      p.textContent = item.snippet ? item.snippet.slice(0, 160) : '';
      card.appendChild(h);
      card.appendChild(meta);
      if(item.snippet) card.appendChild(p);
      card.style.cursor = 'pointer';
      card.addEventListener('click', ()=>item.action && item.action());
      list.appendChild(card);
    }
    globalResultsContainer.appendChild(list);
  }

  if(globalSearchInput){
    globalSearchInput.addEventListener('input', async ()=>{
      await loadGlobalData();
      const results = buildGlobalResults(globalSearchInput.value);
  ...
  setupTagAutocompleteFor('refTags','refTagsSuggestions');
  ...
  // Clear tag filters for references
  const clearRefBtn = document.getElementById('clearRefTagFilters');
  if(clearRefBtn){
