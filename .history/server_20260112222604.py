...
def load_items():
    if USER_FILE.exists():
        try:
            ...
            # Check if we need to remigrate timestamps (all entries have same timestamp)
            needs_remigration = False
            if len(raw) > 1:
                sample = raw[:min(5, len(raw))]
                first_timestamp = sample[0].get('created_at', '')
                if first_timestamp and all(e.get('created_at') == first_timestamp for e in sample):
