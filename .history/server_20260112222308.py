        ...
        try:
            ...
            for idx, it in enumerate(raw):
                ...
                if 'abbreviation' in it or 'type' not in it:
                    new = dict(it)
                    # Add or fix timestamps (re-migrate if all timestamps are identical)
                    existing_created = new.get('created_at', '')
                    # Check if this is a stale migration (all entries have same timestamp)
                    needs_remigration = False
                    if idx == 0 and existing_created and len(raw) > 1:
                        # Sample check: if first 5 entries have identical created_at, remigrate all
                        sample = raw[:min(5, len(raw))]
                        if all(e.get('created_at') == existing_created for e in sample):
